...
BOOST_AUTO_TEST_CASE(over_route)
{
    ...
    unsigned int travelTime{0};
    // route0
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station0.id, station1.id), 1);
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station0.id, station2.id), 1 + 2);
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station0.id, station3.id), 1 + 2 + 3);
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station1.id, station3.id), 2 + 3);
    // route1
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route1.id, station3.id, station1.id), 4);
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route1.id, station3.id, station2.id), 4 + 2);
    // route2
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route2.id, station3.id, station1.id), 4);
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route2.id, station3.id, station0.id), 4 + 1);
    // Invalid routes
    // -- 3 -> 1 is possible, but only over route1 and route2.
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station3.id, station1.id), 0);
    // -- 1 -> 0 is possible, but only over route3.
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station1.id, station0.id), 0);
    BOOST_CHECK_EQUAL(nw.GetTravelTime(line.id, route0.id, station1.id, station1.id), 0);
}
